...
import * as hnd from  './candidate-edit-hnd.mjs'
...
const fl_hrcdt_photo_img = $('#pnl_edit-fl_hrcdt_photo_img');
const fl_hrcdt_photo_lnk = $('#pnl_edit-fl_hrcdt_photo_link');				
				

const pnl_form = $('#pnl_edit-form')
const obj = {
	txt_hrcdt_id: $('#pnl_edit-txt_hrcdt_id'),
	txt_hrcdt_name: $('#pnl_edit-txt_hrcdt_name'),
	fl_hrcdt_photo: $('#pnl_edit-fl_hrcdt_photo'),
	cbo_gender_id: $('#pnl_edit-cbo_gender_id'),
	txt_hrcdt_birthplace: $('#pnl_edit-txt_hrcdt_birthplace'),
	dt_hrcdt_birthdate: $('#pnl_edit-dt_hrcdt_birthdate'),
	txt_hrcdt_ktp: $('#pnl_edit-txt_hrcdt_ktp'),
	txt_hrcdt_address: $('#pnl_edit-txt_hrcdt_address'),
	txt_hrcdt_addressdom: $('#pnl_edit-txt_hrcdt_addressdom'),
	txt_hrcdt_email: $('#pnl_edit-txt_hrcdt_email'),
	txt_hrcdt_handphone: $('#pnl_edit-txt_hrcdt_handphone'),
	txt_hrcdt_telp: $('#pnl_edit-txt_hrcdt_telp'),
...
export async function init(opt) {
	...
	obj.fl_hrcdt_photo.filebox({
		onChange: function(value) {
			var files = obj.fl_hrcdt_photo.filebox('files');
			var f = files[0];
				...
				return function(e) {
					if (loaded.type.startsWith('image')) {
						...
						image.onload = function() {
							fl_hrcdt_photo_img.attr('src', e.target.result);
							fl_hrcdt_photo_img.show();
							fl_hrcdt_photo_lnk.hide();
						}
					} else {
						fl_hrcdt_photo_img.hide();
						fl_hrcdt_photo_lnk.hide();
					}
				}
			})(f);
...
export function createnew() {
	form.createnew(async (data, options)=>{
		...
		data.hrcdt_birthdate = global.now()
		...
		fl_hrcdt_photo_img.hide();
		fl_hrcdt_photo_lnk.hide();	
		obj.fl_hrcdt_photo.filebox('clear');		
		...
		$ui.getPages().ITEMS['pnl_edithrcdtedugrid'].handler.createnew(data, options)
		$ui.getPages().ITEMS['pnl_edithrcdtfamgrid'].handler.createnew(data, options)
		$ui.getPages().ITEMS['pnl_edithrcdttraingrid'].handler.createnew(data, options)
		$ui.getPages().ITEMS['pnl_edithrcdtjobexpgrid'].handler.createnew(data, options)
		$ui.getPages().ITEMS['pnl_edithrcdtorggrid'].handler.createnew(data, options)
		$ui.getPages().ITEMS['pnl_edithrcdtreffgrid'].handler.createnew(data, options)
		$ui.getPages().ITEMS['pnl_edithrcdtattchgrid'].handler.createnew(data, options)
...
function updatefilebox(record) {
	// apabila ada keperluan untuk menampilkan data dari object storage
		...
		obj.fl_hrcdt_photo.filebox('clear');			
		if (record.hrcdt_photo_doc!=undefined) {
			if (record.hrcdt_photo_doc.type.startsWith('image')) {
				fl_hrcdt_photo_lnk.hide();
				fl_hrcdt_photo_img.show();
				fl_hrcdt_photo_img.attr('src', record.hrcdt_photo_doc.attachmentdata);
			} else {
				fl_hrcdt_photo_img.hide();
				fl_hrcdt_photo_lnk.show();
				fl_hrcdt_photo_lnk[0].onclick = () => {
					fl_hrcdt_photo_lnk.attr('download', record.hrcdt_photo_doc.name);
					fl_hrcdt_photo_lnk.attr('href', record.hrcdt_photo_doc.attachmentdata);
				}
			}	
		} else {
			fl_hrcdt_photo_img.hide();
