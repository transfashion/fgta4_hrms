...
import * as hnd from  './candidate-hrcdtreffgrid-hnd.mjs'

const tbl_list = $('#pnl_edithrcdtreffgrid-tbl_list');
const txt_title = $('#pnl_edithrcdtreffgrid-title');
const pnl_control = $('.pnl_edithrcdtreffgrid-control');
const btn_removechecked  = $('#pnl_edithrcdtreffgrid-removechecked');
const btn_addrow = $('#pnl_edithrcdtreffgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/hrcdtreff-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edithrcdtreffform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edithrcdtreffform', () => {
		$ui.getPages().ITEMS['pnl_edithrcdtreffform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/hrcdtreff-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edithrcdtreffform', ()=>{
		$ui.getPages().ITEMS['pnl_edithrcdtreffform'].handler.createnew(header_data)
	})	
}