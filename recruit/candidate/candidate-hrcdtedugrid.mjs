...
import * as hnd from  './candidate-hrcdtedugrid-hnd.mjs'

const tbl_list = $('#pnl_edithrcdtedugrid-tbl_list');
const txt_title = $('#pnl_edithrcdtedugrid-title');
const pnl_control = $('.pnl_edithrcdtedugrid-control');
const btn_removechecked  = $('#pnl_edithrcdtedugrid-removechecked');
const btn_addrow = $('#pnl_edithrcdtedugrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/hrcdtedu-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edithrcdteduform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edithrcdteduform', () => {
		$ui.getPages().ITEMS['pnl_edithrcdteduform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/hrcdtedu-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edithrcdteduform', ()=>{
		$ui.getPages().ITEMS['pnl_edithrcdteduform'].handler.createnew(header_data)
	})	
}