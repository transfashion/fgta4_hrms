...
import * as hnd from  './candidate-hrcdtattchgrid-hnd.mjs'

const tbl_list = $('#pnl_edithrcdtattchgrid-tbl_list');
const txt_title = $('#pnl_edithrcdtattchgrid-title');
const pnl_control = $('.pnl_edithrcdtattchgrid-control');
const btn_removechecked  = $('#pnl_edithrcdtattchgrid-removechecked');
const btn_addrow = $('#pnl_edithrcdtattchgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/hrcdtattch-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edithrcdtattchform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edithrcdtattchform', () => {
		$ui.getPages().ITEMS['pnl_edithrcdtattchform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/hrcdtattch-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edithrcdtattchform', ()=>{
		$ui.getPages().ITEMS['pnl_edithrcdtattchform'].handler.createnew(header_data)
	})	
}