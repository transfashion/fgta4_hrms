...
import * as hnd from  './candidate-hrcdtfamgrid-hnd.mjs'

const tbl_list = $('#pnl_edithrcdtfamgrid-tbl_list');
const txt_title = $('#pnl_edithrcdtfamgrid-title');
const pnl_control = $('.pnl_edithrcdtfamgrid-control');
const btn_removechecked  = $('#pnl_edithrcdtfamgrid-removechecked');
const btn_addrow = $('#pnl_edithrcdtfamgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/hrcdtfam-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edithrcdtfamform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edithrcdtfamform', () => {
		$ui.getPages().ITEMS['pnl_edithrcdtfamform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/hrcdtfam-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edithrcdtfamform', ()=>{
		$ui.getPages().ITEMS['pnl_edithrcdtfamform'].handler.createnew(header_data)
	})	
}