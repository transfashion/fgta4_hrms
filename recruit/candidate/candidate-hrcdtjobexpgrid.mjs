...
import * as hnd from  './candidate-hrcdtjobexpgrid-hnd.mjs'

const tbl_list = $('#pnl_edithrcdtjobexpgrid-tbl_list');
const txt_title = $('#pnl_edithrcdtjobexpgrid-title');
const pnl_control = $('.pnl_edithrcdtjobexpgrid-control');
const btn_removechecked  = $('#pnl_edithrcdtjobexpgrid-removechecked');
const btn_addrow = $('#pnl_edithrcdtjobexpgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.hrcdt_name)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/hrcdtjobexp-list`
		options.criteria['id'] = data.hrcdt_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edithrcdtjobexpform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edithrcdtjobexpform', () => {
		$ui.getPages().ITEMS['pnl_edithrcdtjobexpform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/hrcdtjobexp-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edithrcdtjobexpform', ()=>{
		$ui.getPages().ITEMS['pnl_edithrcdtjobexpform'].handler.createnew(header_data)
	})	
}