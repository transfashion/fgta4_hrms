...
import * as hnd from  './candidate-hrcdtorggrid-hnd.mjs'

const tbl_list = $('#pnl_edithrcdtorggrid-tbl_list');
const txt_title = $('#pnl_edithrcdtorggrid-title');
const pnl_control = $('.pnl_edithrcdtorggrid-control');
const btn_removechecked  = $('#pnl_edithrcdtorggrid-removechecked');
const btn_addrow = $('#pnl_edithrcdtorggrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/hrcdtorg-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edithrcdtorgform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edithrcdtorgform', () => {
		$ui.getPages().ITEMS['pnl_edithrcdtorgform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/hrcdtorg-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edithrcdtorgform', ()=>{
		$ui.getPages().ITEMS['pnl_edithrcdtorgform'].handler.createnew(header_data)
	})	
}