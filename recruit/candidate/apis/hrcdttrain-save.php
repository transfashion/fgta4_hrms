...
if (is_file(__DIR__ .'/data-hrcdttrain-handler.php')) {
	require_once __DIR__ .'/data-hrcdttrain-handler.php';
}
...
/**
 * hrms/recruit/candidate/apis/hrcdttrain-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel hrcdttrain candidate (mst_hrcdttrain)
 *
...
$API = new class extends candidateBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_hrcdttrain';
		$primarykey = 'hrcdttrain_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\candidate_hrcdttrainHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new candidate_hrcdttrainHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->hrcdttrain_name = strtoupper($obj->hrcdttrain_name);
			$obj->hrcdttrain_organizer = strtoupper($obj->hrcdttrain_organizer);
			$obj->hrcdttrain_city = strtoupper($obj->hrcdttrain_city);


			if ($obj->hrcdttrain_descr=='') { $obj->hrcdttrain_descr = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"hrcdttrain_id" => $obj->hrcdttrain_id
				];

				$criteriaValues = [
					"hrcdttrain_id" => " hrcdttrain_id = :hrcdttrain_id "
				];
				...
				$sqlFieldList = [
					'hrcdttrain_id' => 'A.`hrcdttrain_id`', 'hrcdttrain_name' => 'A.`hrcdttrain_name`', 'hrcdttrain_organizer' => 'A.`hrcdttrain_organizer`', 'hrcdttrain_city' => 'A.`hrcdttrain_city`',
					'hrcdttrain_year' => 'A.`hrcdttrain_year`', 'hrcdttrain_descr' => 'A.`hrcdttrain_descr`', 'hrcdt_id' => 'A.`hrcdt_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_hrcdttrain A";
				$sqlWhere = $where->sql;
