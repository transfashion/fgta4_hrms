...
/**
 * hrms/recruit/candidate/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header candidate (mst_hrcdt)
 *
...
$API = new class extends candidateBase {
	...
	public function execute($data, $options, $files) {
		...
		$handlerclassname = "\\FGTA4\\apis\\candidate_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new candidate_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->hrcdt_birthdate = (\DateTime::createFromFormat('d/m/Y',$obj->hrcdt_birthdate))->format('Y-m-d');

			$obj->hrcdt_name = strtoupper($obj->hrcdt_name);
			$obj->gender_id = strtoupper($obj->gender_id);
			$obj->hrcdt_birthplace = strtoupper($obj->hrcdt_birthplace);
			$obj->marital_id = strtoupper($obj->marital_id);
			$obj->religion_id = strtoupper($obj->religion_id);


			if ($obj->hrcdt_telp=='') { $obj->hrcdt_telp = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"hrcdt_id" => $obj->hrcdt_id
				];

				$criteriaValues = [
					"hrcdt_id" => " hrcdt_id = :hrcdt_id "
				];
				...
				$sqlFieldList = [
					'hrcdt_id' => 'A.`hrcdt_id`', 'hrcdt_name' => 'A.`hrcdt_name`', 'hrcdt_photo' => 'A.`hrcdt_photo`', 'gender_id' => 'A.`gender_id`',
					'hrcdt_birthplace' => 'A.`hrcdt_birthplace`', 'hrcdt_birthdate' => 'A.`hrcdt_birthdate`', 'hrcdt_ktp' => 'A.`hrcdt_ktp`', 'hrcdt_address' => 'A.`hrcdt_address`',
					'hrcdt_addressdom' => 'A.`hrcdt_addressdom`', 'hrcdt_email' => 'A.`hrcdt_email`', 'hrcdt_handphone' => 'A.`hrcdt_handphone`', 'hrcdt_telp' => 'A.`hrcdt_telp`',
					'marital_id' => 'A.`marital_id`', 'religion_id' => 'A.`religion_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_hrcdt A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'hrcdt_birthdate' => date("d/m/Y", strtotime($row['hrcdt_birthdate'])),
