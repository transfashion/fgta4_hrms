...
if (is_file(__DIR__ .'/data-hrcdtedu-handler.php')) {
	require_once __DIR__ .'/data-hrcdtedu-handler.php';
}
...
/**
 * hrms/recruit/candidate/apis/hrcdtedu-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel hrcdtedu candidate (mst_hrcdtedu)
 *
...
$API = new class extends candidateBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_hrcdtedu';
		$primarykey = 'hrcdtedu_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\candidate_hrcdteduHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new candidate_hrcdteduHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			if ($obj->hrcdtedu_title=='') { $obj->hrcdtedu_title = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"hrcdtedu_id" => $obj->hrcdtedu_id
				];

				$criteriaValues = [
					"hrcdtedu_id" => " hrcdtedu_id = :hrcdtedu_id "
				];
				...
				$sqlFieldList = [
					'hrcdtedu_id' => 'A.`hrcdtedu_id`', 'edu_id' => 'A.`edu_id`', 'hrcdtedu_name' => 'A.`hrcdtedu_name`', 'hrcdtedu_title' => 'A.`hrcdtedu_title`',
					'hrcdtedu_city' => 'A.`hrcdtedu_city`', 'hrcdtedu_gpa' => 'A.`hrcdtedu_gpa`', 'hrcdt_id' => 'A.`hrcdt_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_hrcdtedu A";
				$sqlWhere = $where->sql;
