...
/**
 * hrms/recruit/candidate/apis/hrcdtreff-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel hrcdtreff candidate (mst_hrcdtreff)
 *
...
$API = new class extends candidateBase {
	...
	public function execute($data, $options, $files) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new candidate_hrcdtreffHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->hrcdtreff_name = strtoupper($obj->hrcdtreff_name);


			if ($obj->hrcdtreff_handphone=='') { $obj->hrcdtreff_handphone = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"hrcdtreff_id" => $obj->hrcdtreff_id
				];

				$criteriaValues = [
					"hrcdtreff_id" => " hrcdtreff_id = :hrcdtreff_id "
				];
				...
				$sqlFieldList = [
					'hrcdtreff_id' => 'A.`hrcdtreff_id`', 'hrcdtreff_name' => 'A.`hrcdtreff_name`', 'hrcdtreff_relation' => 'A.`hrcdtreff_relation`', 'hrcdtreff_handphone' => 'A.`hrcdtreff_handphone`',
					'hrcdt_id' => 'A.`hrcdt_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_hrcdtreff A";
				$sqlWhere = $where->sql;
