...
/**
 * hrms/recruit/candidate/apis/hrcdtfam-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends candidateBase {
	...
	public function execute($data, $options, $files) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new candidate_hrcdtfamHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->hrcdtfam_birthdate = (\DateTime::createFromFormat('d/m/Y',$obj->hrcdtfam_birthdate))->format('Y-m-d');

			$obj->hrcdtfam_name = strtoupper($obj->hrcdtfam_name);
			$obj->gender_id = strtoupper($obj->gender_id);
			$obj->hrcdtfam_birthplace = strtoupper($obj->hrcdtfam_birthplace);


			if ($obj->hrcdtfam_birthplace=='') { $obj->hrcdtfam_birthplace = '--NULL--'; }
			if ($obj->hrcdtfam_birthdate=='') { $obj->hrcdtfam_birthdate = '--NULL--'; }
			if ($obj->hrcdtfam_handphone=='') { $obj->hrcdtfam_handphone = '--NULL--'; }
			...
			try {
				...
				$dataresponse = array_merge($record, [
					...
					'hrcdtfam_birthdate' => date("d/m/Y", strtotime($row['hrcdtfam_birthdate'])),
