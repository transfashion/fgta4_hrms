...
if (is_file(__DIR__ .'/data-hrcdtorg-handler.php')) {
	require_once __DIR__ .'/data-hrcdtorg-handler.php';
}
...
/**
 * hrms/recruit/candidate/apis/hrcdtorg-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel hrcdtorg candidate (mst_hrcdtorg)
 *
...
$API = new class extends candidateBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_hrcdtorg';
		$primarykey = 'hrcdtorg_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\candidate_hrcdtorgHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new candidate_hrcdtorgHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->hrcdtorg_name = strtoupper($obj->hrcdtorg_name);
			$obj->hrcdtorg_position = strtoupper($obj->hrcdtorg_position);


			if ($obj->hrcdtorg_descr=='') { $obj->hrcdtorg_descr = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"hrcdtorg_id" => $obj->hrcdtorg_id
				];

				$criteriaValues = [
					"hrcdtorg_id" => " hrcdtorg_id = :hrcdtorg_id "
				];
				...
				$sqlFieldList = [
					'hrcdtorg_id' => 'A.`hrcdtorg_id`', 'hrcdtorg_name' => 'A.`hrcdtorg_name`', 'hrcdtorg_city' => 'A.`hrcdtorg_city`', 'hrcdtorg_position' => 'A.`hrcdtorg_position`',
					'hrcdtorg_descr' => 'A.`hrcdtorg_descr`', 'hrcdt_id' => 'A.`hrcdt_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_hrcdtorg A";
				$sqlWhere = $where->sql;
