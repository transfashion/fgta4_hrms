...
if (is_file(__DIR__ .'/data-hrcdtattch-handler.php')) {
	require_once __DIR__ .'/data-hrcdtattch-handler.php';
}
...
/**
 * hrms/recruit/candidate/apis/hrcdtattch-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel hrcdtattch candidate (mst_hrcdtattch)
 *
...
$API = new class extends candidateBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_hrcdtattch';
		$primarykey = 'hrcdtattch_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\candidate_hrcdtattchHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new candidate_hrcdtattchHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->hrcdtattch_validity = (\DateTime::createFromFormat('d/m/Y',$obj->hrcdtattch_validity))->format('Y-m-d');
			$obj->hrcdtattch_expired = (\DateTime::createFromFormat('d/m/Y',$obj->hrcdtattch_expired))->format('Y-m-d');



			if ($obj->hrcdtattch_expired=='') { $obj->hrcdtattch_expired = '--NULL--'; }
			...
			try {
				...
				if (property_exists($files, $fieldname)) {
					...
					$objfile->hrcdtattch_filename = $rev;
				...
				// result
				$options->criteria = [
					"hrcdtattch_id" => $obj->hrcdtattch_id
				];

				$criteriaValues = [
					"hrcdtattch_id" => " hrcdtattch_id = :hrcdtattch_id "
				];
				...
				$sqlFieldList = [
					'hrcdtattch_id' => 'A.`hrcdtattch_id`', 'hrcdtattch_name' => 'A.`hrcdtattch_name`', 'hrcdtattch_filename' => 'A.`hrcdtattch_filename`', 'hrcdtattch_validity' => 'A.`hrcdtattch_validity`',
					'hrcdtattch_expired' => 'A.`hrcdtattch_expired`', 'hrcdtdate_id' => 'A.`hrcdtdate_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_hrcdtattch A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'hrcdtattch_validity' => date("d/m/Y", strtotime($row['hrcdtattch_validity'])),
					'hrcdtattch_expired' => date("d/m/Y", strtotime($row['hrcdtattch_expired'])),
