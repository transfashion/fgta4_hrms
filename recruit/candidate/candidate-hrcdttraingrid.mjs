...
import * as hnd from  './candidate-hrcdttraingrid-hnd.mjs'

const tbl_list = $('#pnl_edithrcdttraingrid-tbl_list');
const txt_title = $('#pnl_edithrcdttraingrid-title');
const pnl_control = $('.pnl_edithrcdttraingrid-control');
const btn_removechecked  = $('#pnl_edithrcdttraingrid-removechecked');
const btn_addrow = $('#pnl_edithrcdttraingrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/hrcdttrain-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edithrcdttrainform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edithrcdttrainform', () => {
		$ui.getPages().ITEMS['pnl_edithrcdttrainform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/hrcdttrain-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edithrcdttrainform', ()=>{
		$ui.getPages().ITEMS['pnl_edithrcdttrainform'].handler.createnew(header_data)
	})	
}