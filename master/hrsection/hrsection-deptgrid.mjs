...
const tbl_list = $('#pnl_editdeptgrid-tbl_list')
const txt_title = $('#pnl_editdeptgrid-title')
const pnl_control = $('#pnl_editdeptgrid-control')
const btn_removechecked  = $('#pnl_editdeptgrid-removechecked')
const btn_addrow = $('#pnl_editdeptgrid-addrow')
...
export function createnew(data, options) {
	...
	grd_list.clear()
	txt_title.html(data.hrsection_name)
	header_data = data
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/dept-list`
		options.criteria['id'] = data.hrsection_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editdeptform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editdeptform', () => {
		$ui.getPages().ITEMS['pnl_editdeptform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/dept-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editdeptform', ()=>{
		$ui.getPages().ITEMS['pnl_editdeptform'].handler.createnew(header_data)
	})	
}