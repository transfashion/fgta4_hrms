...
/**
 * hrms/master/hrrel/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header hrrel (mst_hrrel)
 *
 ...
 * tanggal 25/02/2023
 */
$API = new class extends hrrelBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\hrrel_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new hrrel_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->hrrel_id = strtoupper($obj->hrrel_id);
			$obj->hrrel_name = strtoupper($obj->hrrel_name);
			...
			try {
				...
				// result
				$options->criteria = [
					"hrrel_id" => $obj->hrrel_id
				];

				$criteriaValues = [
					"hrrel_id" => " hrrel_id = :hrrel_id "
				];
				...
				$sqlFieldList = [
					'hrrel_id' => 'A.`hrrel_id`', 'hrrel_name' => 'A.`hrrel_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_hrrel A";
				$sqlWhere = $where->sql;
