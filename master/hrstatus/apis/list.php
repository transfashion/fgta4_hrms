...
/**
 * hrms/master/hrstatus/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header hrstatus (mst_hrstatus)
...
$API = new class extends hrstatusBase {
	...
	public function execute($options) {
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.hrstatus_id LIKE CONCAT('%', :search, '%') OR A.hrstatus_name LIKE CONCAT('%', :search, '%') "
			];
			if (is_object($hnd)) {
