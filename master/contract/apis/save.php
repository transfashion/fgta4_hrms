...
/**
 * hrms/master/contract/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header contract (mst_hrcont)
 *
 ...
 * tanggal 17/03/2023
 */
$API = new class extends contractBase {
	
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_hrcont';
		$primarykey = 'hrcont_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\contract_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new contract_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->hrcont_filedoc = strtoupper($obj->hrcont_filedoc);
			$obj->hrcont_nodoc = strtoupper($obj->hrcont_nodoc);
			...
			try {
				...
				// result
				$options->criteria = [
					"hrcont_id" => $obj->hrcont_id
				];

				$criteriaValues = [
					"hrcont_id" => " hrcont_id = :hrcont_id "
				];
				...
				$sqlFieldList = [
					'hrcont_id' => 'A.`hrcont_id`', 'hrcont_filedoc' => 'A.`hrcont_filedoc`', 'hrcont_nodoc' => 'A.`hrcont_nodoc`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_hrcont A";
				$sqlWhere = $where->sql;
