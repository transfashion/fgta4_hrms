...
import * as hnd from  './contract-edit-hnd.mjs'
...
const fl_hrcont_filedoc_img = $('#pnl_edit-fl_hrcont_filedoc_img');
const fl_hrcont_filedoc_lnk = $('#pnl_edit-fl_hrcont_filedoc_link');				
				

const pnl_form = $('#pnl_edit-form')
const obj = {
	txt_hrcont_id: $('#pnl_edit-txt_hrcont_id'),
	fl_hrcont_filedoc: $('#pnl_edit-fl_hrcont_filedoc'),
	txt_hrcont_nodoc: $('#pnl_edit-txt_hrcont_nodoc')
}
...
export async function init(opt) {
	...
	obj.fl_hrcont_filedoc.filebox({
		onChange: function(value) {
			var files = obj.fl_hrcont_filedoc.filebox('files');
			var f = files[0];
				...
				return function(e) {
					if (loaded.type.startsWith('image')) {
						...
						image.onload = function() {
							fl_hrcont_filedoc_img.attr('src', e.target.result);
							fl_hrcont_filedoc_img.show();
							fl_hrcont_filedoc_lnk.hide();
						}
					} else {
						fl_hrcont_filedoc_img.hide();
						fl_hrcont_filedoc_lnk.hide();
					}
				}
			})(f);
...
export function createnew() {
	form.createnew(async (data, options)=>{
		...
		fl_hrcont_filedoc_img.hide();
		fl_hrcont_filedoc_lnk.hide();	
		obj.fl_hrcont_filedoc.filebox('clear');		
...
function updatefilebox(record) {
	// apabila ada keperluan untuk menampilkan data dari object storage
		...
		obj.fl_hrcont_filedoc.filebox('clear');			
		if (record.hrcont_filedoc_doc!=undefined) {
			if (record.hrcont_filedoc_doc.type.startsWith('image')) {
				fl_hrcont_filedoc_lnk.hide();
				fl_hrcont_filedoc_img.show();
				fl_hrcont_filedoc_img.attr('src', record.hrcont_filedoc_doc.attachmentdata);
			} else {
				fl_hrcont_filedoc_img.hide();
				fl_hrcont_filedoc_lnk.show();
				fl_hrcont_filedoc_lnk[0].onclick = () => {
					fl_hrcont_filedoc_lnk.attr('download', record.hrcont_filedoc_doc.name);
					fl_hrcont_filedoc_lnk.attr('href', record.hrcont_filedoc_doc.attachmentdata);
				}
			}	
		} else {
			fl_hrcont_filedoc_img.hide();
