...
const tbl_list = $('#pnl_editmembergrid-tbl_list');
const txt_title = $('#pnl_editmembergrid-title');
const pnl_control = $('#pnl_editmembergrid-control');
const btn_removechecked  = $('#pnl_editmembergrid-removechecked');
const btn_addrow = $('#pnl_editmembergrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/member-list`
		options.criteria['id'] = data.team_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editmemberform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editmemberform', () => {
		$ui.getPages().ITEMS['pnl_editmemberform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/member-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editmemberform', ()=>{
		$ui.getPages().ITEMS['pnl_editmemberform'].handler.createnew(header_data)
	})	
}