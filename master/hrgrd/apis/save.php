...
/**
 * hrms/master/hrgrd/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header hrgrd (mst_hrgrd)
 *
 ...
 * tanggal 05/02/2023
 */
$API = new class extends hrgrdBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\hrgrd_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new hrgrd_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->hrgrd_name = strtoupper($obj->hrgrd_name);


			if ($obj->hrgrd_group=='') { $obj->hrgrd_group = '--NULL--'; }
			if ($obj->hrgrd_descr=='') { $obj->hrgrd_descr = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"hrgrd_id" => $obj->hrgrd_id
				];

				$criteriaValues = [
					"hrgrd_id" => " hrgrd_id = :hrgrd_id "
				];
				...
				$sqlFieldList = [
					'hrgrd_id' => 'A.`hrgrd_id`', 'hrgrd_name' => 'A.`hrgrd_name`', 'hrgrd_group' => 'A.`hrgrd_group`', 'hrgrd_descr' => 'A.`hrgrd_descr`',
					'hrgrd_order' => 'A.`hrgrd_order`', 'hrgrd_value' => 'A.`hrgrd_value`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_hrgrd A";
				$sqlWhere = $where->sql;
