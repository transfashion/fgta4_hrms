...
const tbl_list = $('#pnl_editunitgrid-tbl_list')
const txt_title = $('#pnl_editunitgrid-title')
const pnl_control = $('#pnl_editunitgrid-control')
const btn_removechecked  = $('#pnl_editunitgrid-removechecked')
const btn_addrow = $('#pnl_editunitgrid-addrow')
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/unit-list`
		options.criteria['id'] = data.empl_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editunitform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editunitform', () => {
		$ui.getPages().ITEMS['pnl_editunitform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/unit-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editunitform', ()=>{
		$ui.getPages().ITEMS['pnl_editunitform'].handler.createnew(header_data)
	})	
}