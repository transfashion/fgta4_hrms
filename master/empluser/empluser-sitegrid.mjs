...
const tbl_list = $('#pnl_editsitegrid-tbl_list')
const txt_title = $('#pnl_editsitegrid-title')
const pnl_control = $('#pnl_editsitegrid-control')
const btn_removechecked  = $('#pnl_editsitegrid-removechecked')
const btn_addrow = $('#pnl_editsitegrid-addrow')
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/site-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editsiteform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editsiteform', () => {
		$ui.getPages().ITEMS['pnl_editsiteform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/site-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editsiteform', ()=>{
		$ui.getPages().ITEMS['pnl_editsiteform'].handler.createnew(header_data)
	})	
}