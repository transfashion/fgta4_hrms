...
import * as hnd from  './hrjob-hrjobrolegrid-hnd.mjs'

const tbl_list = $('#pnl_edithrjobrolegrid-tbl_list');
const txt_title = $('#pnl_edithrjobrolegrid-title');
const pnl_control = $('.pnl_edithrjobrolegrid-control');
const btn_removechecked  = $('#pnl_edithrjobrolegrid-removechecked');
const btn_addrow = $('#pnl_edithrjobrolegrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.hrjob_name)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/hrjobrole-list`
		options.criteria['id'] = data.hrjob_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edithrjobroleform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edithrjobroleform', () => {
		$ui.getPages().ITEMS['pnl_edithrjobroleform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/hrjobrole-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edithrjobroleform', ()=>{
		$ui.getPages().ITEMS['pnl_edithrjobroleform'].handler.createnew(header_data)
	})	
}