...
if (is_file(__DIR__ .'/data-hrjobrole-handler.php')) {
	require_once __DIR__ .'/data-hrjobrole-handler.php';
}
...
/**
 * hrms/master/hrjob/apis/hrjobrole-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel hrjobrole hrjob (mst_hrjobrole)
 *
 ...
 * tanggal 05/02/2023
 */
$API = new class extends hrjobBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_hrjobrole';
		$primarykey = 'hrjobrole_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\hrjob_hrjobroleHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new hrjob_hrjobroleHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$header_table = 'mst_hrjob';
				$header_primarykey = 'hrjob_id';
				...
				// result
				$options->criteria = [
					"hrjobrole_id" => $obj->hrjobrole_id
				];

				$criteriaValues = [
					"hrjobrole_id" => " hrjobrole_id = :hrjobrole_id "
				];
				...
				$sqlFieldList = [
					'hrjobrole_id' => 'A.`hrjobrole_id`', 'role_id' => 'A.`role_id`', 'hrjob_id' => 'A.`hrjob_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_hrjobrole A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'role_name' => \FGTA4\utils\SqlUtility::Lookup($record['role_id'], $this->db, 'fgt_role', 'role_id', 'role_name'),
