...
/**
 * hrms/master/hrjob/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header hrjob (mst_hrjob)
...
$API = new class extends hrjobBase {
	...
	public function execute($options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\hrjob_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new hrjob_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.hrjob_id LIKE CONCAT('%', :search, '%') OR A.hrjob_name LIKE CONCAT('%', :search, '%') "
			];
