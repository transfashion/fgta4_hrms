...
/**
 * hrms/master/hrjob/apis/hrjobrole-list.php
 *
 * ==============
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel hrjobrole hrjob (mst_hrjobrole)
...
$API = new class extends hrjobBase {
	...
	public function execute($options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new hrjob_hrjobroleHandler($options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('role_name', 'role_id', $record, 'fgt_role', 'role_name', 'role_id');
