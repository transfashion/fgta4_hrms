...
/**
 * hrms/hrd/hremplmut/apis/approval-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends hremplmutBase {
	...
	public function execute($data, $options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new hremplmut_approvalHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$header_table = 'trn_hremplmut';
				$header_primarykey = 'hremplmut_id';
				...
				// result
				$options->criteria = [
					"hremplmutappr_id" => $obj->hremplmutappr_id
				];
