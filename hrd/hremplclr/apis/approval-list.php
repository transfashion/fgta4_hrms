...
/**
 * hrms/hrd/hremplclr/apis/approval-list.php
 *
 * ==============
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel approval hremplclr (trn_hremplclrappr)
...
$API = new class extends hremplclrBase {
	...
	public function execute($options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new hremplclr_approvalHandler($options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
			...
			foreach ($rows as $row) {
				...
				$record = array_merge($record, [
					...
					//'tanggal' => date("d/m/y", strtotime($record['tanggal'])),
				 	//'tambahan' => 'dta'
				'hremplclrappr_by' => \FGTA4\utils\SqlUtility::Lookup($record['hremplclrappr_by'], $this->db, $GLOBALS['MAIN_USERTABLE'], 'user_id', 'user_fullname'),
				'hremplclrappr_declinedby' => \FGTA4\utils\SqlUtility::Lookup($record['hremplclrappr_declinedby'], $this->db, $GLOBALS['MAIN_USERTABLE'], 'user_id', 'user_fullname'),
				...
				*/
				...
				$this->addFields('undefined', 'hremplclrappr_by', $record, $GLOBALS['MAIN_USERTABLE'], 'user_fullname', 'user_id');
				$this->addFields('undefined', 'hremplclrappr_declinedby', $record, $GLOBALS['MAIN_USERTABLE'], 'user_fullname', 'user_id');
